    ...
    return filenames
        ...
        .map(file => {
            ...
            const mapped = paintArray
                .filter(paint => paint.price.price <= 3.75 || paint.price.price === 4.24 || paint.price.price === 13.99 || paint.price.price === 12.99)
                .map(paint => ({
                name: paint.title.split("-")[0].split(capitalizeWords(fileNameWithoutExt))[0].trim(),
